...
/*
	Model State
*/
enum ShipModelState 	{ SPAWN, FLYING, WALKING, DEAD, };
...
class ShipModel : GameComponent
{
	...
	bool isAlive() const 	{ return m_currentState != ShipModelState::DEAD; }
	...
	private void onThink(const float dt) override
	{
		switch(m_currentState)
		{
			case(ShipModelState::SPAWN):
			{
				...
				m_currentState = ShipModelState::FLYING;

				break;
			} // Ship Spawn State

			case(ShipModelState::FLYING):
			{
				...
				// Calculate the tilt.
				{
					...
					const float tilt = (m_tilt * scale) * dt;

					m_currentTilt += tilt;
					m_currentTilt *= 0.9f;

					getVortexTransform().setTilt(m_currentTilt);
				...
				// Jump the ship up.
				{
					if(isJumping())
					{
						...
						{
							getVortexTransform().setDepth(m_groundLevel);
				...
				// Damage or Destroy Ship
				{
					if(m_damageShip >= m_currentHealthParam)
					{
						...
						m_currentState = ShipModelState::DEAD;
					}
					else
					{
	...
	private float 				m_currentTilt			= 0;
	private ShipModelState 		m_currentState 			= ShipModelState::SPAWN;
};
