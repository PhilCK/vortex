...
class CharacterController : GameComponent
{
	...
	private void onThink(const float dt) override
	{
		...
		if(transform !is null && input !is null)
		{
			...
			//Position
			{
				...
				// Throw
				if(input.isDownOnFrame(Input::MS_LEFT))
				{
					Entity @progCube = getOwner().getState().pushNewEntity("ProgramCube");

					if(progCube !is null)
					{
						ProgramableCubeController @cubeController = progCube.getComponent("ProgramableCubeController");

						// Set the throw direction and position.
						{
							// Direction
							const float throwPower 			   = 1000.f;
							const Math::Vec3 throwDir 		   = transform.getForwardVec().add(transform.getUpVec());
							const Math::Vec3 throwDirWithPower = (throwDir.normalize()).scale(throwPower);

							// Position
							const Math::Vec3 position = transform.getPosition().add(transform.getForwardVec().scale(1.5f));

							cubeController.throwCube(throwDirWithPower, position);
						}
					}
					else
					{
						Util::LogError("CharacterController - Wasn't able to create a throwable cube");
					}
				}
			} //Position
		}
	...
	private void onKeyboardChange(const KeyState state, const Key key)
	{
